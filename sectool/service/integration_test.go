...
func TestBurpBackendRules_Integration(t *testing.T) {
	...
	t.Run("add_rule", func(t *testing.T) {
		rule, err := backend.AddRule(t.Context(), false, ProxyRuleInput{
	...
	t.Run("duplicate_label_rejected", func(t *testing.T) {
		...
		_, err = backend.AddRule(t.Context(), false, ProxyRuleInput{
			Label: "integration-test-duplicate",
			Type:  "request_header",
		})
		require.Error(t, err)
	...
	t.Run("update_to_duplicate_label", func(t *testing.T) {
		// Add a second rule
		rule2, err := backend.AddRule(t.Context(), false, ProxyRuleInput{
